...
const Home = () => (
  <div className="main-content home active">
...
let minsers = TeacherList.map((item, index) => {
    return (
          <li key={index} className="teacher">
                <img className="teacher-img" src={item.img_src} alt="" />
                <h3>{item.name}</h3>
                <p>{item.bio}</p>
          </li>
    )
...
const NotFoundPage = (props) => {
   return (
      <div className="main-content"> <h2 className="title"> Error 404! </h2> </div>
   );
}
